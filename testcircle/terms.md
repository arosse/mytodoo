# Terms & Conditions
*Last updated: July 2026*

Welcome to **TestCircle**, a peer-to-peer app testing platform developed by **GreenAnt Apps**. These Terms & Conditions ("Terms") govern your use of the app. By using TestCircle, you agree to be bound by these Terms.

## 1. Eligibility
TestCircle is intended for adult Android developers (18+) who are seeking to complete Google Play's closed testing requirement. By using the platform, you confirm that you are the developer or authorised representative of the app you submit.

## 2. Google Account Requirements & Google Group Setup
To participate in TestCircle, two separate Google Group actions are required:

**Action 1 — Join the Master Google Group**
Join the TestCircle Master Google Group at **https://groups.google.com/g/testcircle-testers** using the Google account linked to the Android device you will use for testing. This allows you to install other testers' apps from the Play Store.
**Action 2 — Add the Master Google Group to your Play Console**
In your Google Play Console, add **testcircle-testers@googlegroups.com** as a tester on your app's closed testing track. This allows other testers to install your app.

Both actions are mandatory. Failing either one will break the mutual testing cycle:
*   Missing Action 1 → you cannot install other testers' apps.
*   Missing Action 2 → other testers cannot install your app.

**Note on accounts:** You may sign in to TestCircle with any Google account. However, Action 1 must be performed using the Google account linked to your Android test device — this may be a different account to the one you use to sign in to TestCircle. Using the wrong account for Action 1 will prevent you from installing other testers' apps, but will not affect other testers' ability to install your app.

## 3. How the Platform Works
TestCircle offers two distinct methods for app testing:

### A. Group Tests
TestCircle matches developers into synchronized testing groups. 
*   A single user account can only participate in one active Group Test at a time. You may join or start a new Group Test under that account only after your current testing cycle fully completes.
*   Each member agrees to install and genuinely test all apps assigned to their testing group.
*   Each member agrees to submit honest, good-faith feedback during each testing phase and complete all mandatory tasks before their respective deadlines.

### B. Single Tests
Users can list their apps individually or test other developers' apps standalone to earn in-app Credits. When testing an app in the Single Tests track:
*   You must install and test the app for the required duration specified in the platform.
*   You are required to provide a screenshot within the app as valid proof of your installation and testing effort.
*   Upon completion, you will be prompted to leave either an honest Play Store review or a private in-app review. Review submission choice is entirely voluntary, and users are never forced to submit public Play Store ratings.
*   Each single app can only be tested by a user once. If a test cannot be fully completed due to external technical issues (e.g., a broken Play Store link, local availability constraints, missing test credentials, or language barriers), a partial Credit reward may be granted for the attempted effort. If a partial reward is claimed, you will not be allowed to re-test that specific app in the future, even if the developer updates or fixes the submission.

## 4. App Submissions & Visibility Boosts
By submitting your app to TestCircle, you confirm that:
*   You are the developer or authorised representative of the submitted app.
*   The app is available for download on the Google Play Store.
*   The app is available in all countries (testers may be from any region).
*   The app does not contain illegal, harmful, or inappropriate content.
*   Any test credentials you provide are valid and allow access to the app's core features.
*   **Duplicate and Group Restrictions:** A single app (identified by its unique package name) cannot have multiple active submissions running simultaneously within the same specific testing group or Single Tests slot. However, developers may submit the same app to a Group Test and a Single Test track at the same time. Furthermore, each Group Test slot must represent a unique physical testing participant. Attempting to occupy multiple slots within the same specific Group Test using different accounts or apps on a single physical device reduces the unique tester pool for other members and is strictly prohibited. Any setup that manipulates the system to deny other group members unique hardware testers will result in immediate group exclusion and account suspension.

### Visibility, Credits, and In-App Purchases (IAP)
Developers can increase traffic to their Single Tests submissions through two methods:
*   **Paid IAP Boosts:** Users can purchase time-limited visibility tiers using real money via In-App Purchases. These tiers position the app near the top of the directory lists and automatically increase the Credit payout reward given to other testers who complete the assignment. Visibility placement is strictly prioritized by tier level; selecting a more premium visibility tier will always ensure placement above lower-tier paid boosts. Once the purchased time window expires, the app reverts to standard chronological sorting and default payout values.
*   **Credit Boosts:** Alternatively, users can spend earned in-app Credits to "boost" their app for free. This action refreshes the submission timestamp, moving the app toward the upper section of the directory.
*   **Organic Sorting:** Standard, un-boosted apps are displayed in strict chronological order (most recent at the top). Organic and Credit-boosted apps will always be positioned below active, premium IAP-boosted apps.

GreenAnt Apps reserves the right to remove any app from the platform that violates our content or submission requirements.

## 5. User Conduct & In-App Communication
TestCircle includes peer feedback loops and an integrated, limited group messaging system designed to help developers coordinate, clarify bugs, and share testing details. You explicitly agree not to misuse these systems. 

Prohibited behavior includes, but is not limited to:
*   Submitting fake, automated, low-effort, or copy-pasted testing feedback.
*   Uploading fraudulent, edited, or irrelevant screenshots as proof of testing.
*   Attempting to exploit or game the mutual testing metrics or Credit reward system.
*   Harassing, abusing, intimidating, threatening, or sending inappropriate/unrelated text or media content to other platform users through feedback forms or within the in-app chat system.
*   Utilising platform feedback forms, chat messages, or internal communication channels to advertise, solicit, promote, or redirect TestCircle users to external testing groups, third-party platforms, or competing testing arrangements.

### Enforcement, Bans, and Strict No-Refund Policy
We maintain a zero-tolerance policy for platform abuse. Any violations of user conduct guidelines can result in immediate penalties, including:
1.  Immediate removal from active Group Tests.
2.  Temporary suspension or permanent ban of your account from accessing TestCircle entirely.
3.  Forfeiture of all accumulated data, app listings, and testing progression.

**Strict No-Refund Rule:** If your account is suspended, restricted, or permanently banned due to conduct violations, or if you choose to voluntarily delete your account, **absolutely no refunds of any sort will be issued**. This applies to all outstanding virtual balances, unused in-app Credits, and active premium In-App Purchases (IAP) visibility campaigns.

## 6. Test Credentials
If you provide test account credentials for your app, you acknowledge that these will be shared with developers in your testing group who have confirmed a mutual install. Do not provide credentials that give access to sensitive personal data beyond what is necessary for testing.

## 7. AI-Generated Summaries
At the end of a testing cycle, TestCircle uses Google's Gemini AI to generate a summary of feedback received on your app. This summary is generated from anonymised feedback text only. GreenAnt Apps does not guarantee the accuracy, completeness, or usefulness of AI-generated summaries. They are provided as a convenience tool only.

## 8. Google Play Compliance
TestCircle facilitates the closed testing process but does not guarantee that use of the platform will result in Google Play production access. Meeting Google's requirements (including the 12-tester, 14-day rule) depends on factors outside our control. GreenAnt Apps is not responsible for any Google Play policy decisions.

## 9. Account Deletion
You may delete your account at any time from within the app (Settings → Delete Account). Upon deletion, all your personal data and testing history will be permanently removed. Voluntary account deletion results in the immediate forfeiture of all remaining virtual Credits or active purchase boosts without any right to a refund.

## 10. Intellectual Property
All content in TestCircle, including the app design, graphics, and branding, is the property of **GreenAnt Apps** or its licensors. You may not modify, reproduce, or distribute any part of the app without prior written permission.

User-submitted content (feedback, screenshots) remains your property. By submitting it, you grant GreenAnt Apps a limited licence to store and display it within the platform for the purposes of operating the service.

Detailed phase feedback is automatically deleted 30 days after a testing cycle completes. Screenshots are automatically deleted after 30 days. AI-generated summaries are retained for as long as your account remains active.

## 11. Disclaimer
TestCircle is provided "as is" without warranties of any kind. While we aim to provide a reliable and fair platform, GreenAnt Apps is not responsible for:
*   Testing group outcomes or Google Play decisions.
*   Data loss due to technical issues.
*   Actions of other users on the platform.
*   Accuracy of AI-generated feedback summaries.

## 12. Changes to These Terms
We may update these Terms occasionally. When we do, the updated version will be published at this URL and the version date will be updated. Continued use of the app after changes constitutes acceptance of the updated Terms.

## 13. Contact
If you have any questions or concerns, feel free to contact us at:

**greenantapps@gmail.com**